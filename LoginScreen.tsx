...
import { getEmployees, getTenants, getSuperAdmin, getPartners } from './services/api';
import type { Employee, EmployeeProfile, Tenant, SuperAdmin, Partner } from './types';
...
interface LoginScreenProps {
  ...
  onPartnerLogin: (user: Partner) => void;
}
...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, onSuperAdminLogin, onPartnerLogin }) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const [{ data: tenantList }, superAdminUser, partnerList] = await Promise.all([
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-brand-primary">NorthStar HCM</h1>
      ...
      <Card title="Log in" className="w-full max-w-4xl">
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            {partners.length > 0 && (
                <div>
                    <h3 className="text-lg font-semibold text-slate-700 mb-3 border-b pb-2 text-center">Partner Portal</h3>
                    <div className="flex flex-wrap justify-center gap-4">
                        {partners.map(partner => (
                            <div
                                key={partner.id}
                                onClick={() => onPartnerLogin(partner)}
                                className="flex flex-col items-center p-4 border border-slate-200 rounded-lg hover:bg-brand-light hover:border-brand-primary cursor-pointer transition-all duration-150 w-48"
                            >
                                <Avatar name={partner.name} size="md" />
                                <p className="font-semibold text-slate-800 mt-2 text-center">{partner.name}</p>
                            </div>
                        ))}
                    </div>
                </div>
